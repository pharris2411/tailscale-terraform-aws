...
data "aws_iam_policy_document" "ecs-cluster-runner-policy" {
  statement {
    actions = ["ec2:Describe*", "ecr:Describe*", "ecr:BatchGet*", "ecs:*"]
    resources = ["*"]
  }
  statement {
    actions = ["ecs:*"]
  ...
  statement {
    actions = ["ecs:*"]
    resources = ["arn:aws:ecs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:cluster/${var.app_name}"]
  }
}
...
resource "aws_instance" "ecs-cluster-runner" {
  ...
  user_data = <<-EOF
    #!/bin/bash
    echo ECS_CLUSTER=${aws_ecs_cluster.aws-ecs.name} >> /etc/ecs/ecs.config
...
resource "aws_security_group_rule" "ecs-cluster-host-http" {
  security_group_id = aws_security_group.ecs-cluster-host.id
  description = "http access to ecs cluster"
  type = "ingress"
  from_port = 80
  to_port = 80
  protocol = "tcp"
...
resource "aws_security_group_rule" "ecs-cluster-host-https" {
  security_group_id = aws_security_group.ecs-cluster-host.id
  description = "https access to ecs cluster"
  type = "ingress"
  from_port = 443
  to_port = 443
  protocol = "tcp"
